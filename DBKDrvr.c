	...
	--*/
{
	...
	/* Initialize SvmDebug bridge (non-fatal if SvmDebug not loaded) */
	DbgPrint("[DBK-SVM] About to call SvmBridge_Init()...\n");
	{
		NTSTATUS svmStatus = SvmBridge_Init();
		DbgPrint("[DBK-SVM] SvmBridge_Init returned 0x%X, Active=%d\n",
	...
	IN PIRP Irp)
{
	...
	if (SeSinglePrivilegeCheck(sedebugprivUID, UserMode))
	{
		...
		DbgPrint("[DBK-SVM] DispatchCreate: PID=%llu opened device, SvmActive=%d\n",
			(ULONG64)(ULONG_PTR)PsGetCurrentProcessId(), SvmBridge_IsActive());

		/* Auto-register CE process with SvmDebug */
		if (SvmBridge_IsActive()) {
			NTSTATUS regStatus = SvmBridge_RegisterCE(PsGetCurrentProcessId());
			DbgPrint("[DBK-SVM] RegisterCE PID=%llu result=0x%X\n",
				(ULONG64)(ULONG_PTR)PsGetCurrentProcessId(), regStatus);
		}
#ifdef AMD64
#ifdef TOBESIGNED
		{
